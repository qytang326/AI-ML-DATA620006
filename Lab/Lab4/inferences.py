    ...
    def sum_out(self, var):
        ...
        cpt = {
            event_values(e, variables): sum(
                self.p(extend(e, var, val)) for val in [True, False])
            for e in all_events(variables, {})
        }
        return Factor(variables, cpt)
...
def conditional_probability(x_group, parent_vars, all_evidence,
                            parent_evidence, children_TorF_tuple):
    if len(x_group) > 1:
    ...
    if '|' not in content:  # joint/ marginal distribution
        ...
        if ',' in children:
            ...
            if '=' in children:
                ...
                x_group = [child]
        prob_enumeration, prob_elimination = conditional_probability(
            x_group, parent_vars, all_evidence, parent_evidence,
            children_TorF_tuple)
                ...
                if '|' not in line:  # no parent node
                    ...
                    for i in range(int(2**len(every_parents))):
                        ...
                        if len(match) == 4:
                            ...
                            cpt[(match[1], match[2], match[3])] = float(
                                match[0])
                        line = input().strip()
                    if not line:
