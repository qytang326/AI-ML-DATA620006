...
static void do_command()
{
    ...
    if((param=get_cmd_param("info", cmd))!=0) {
        ...
        if((info=get_cmd_param("timeout_match", param))!=0)
        {
            info_timeout_match=atoi(info);
            set_timeout_match(info_timeout_match);
        }
        if((info=get_cmd_param("timeout_turn", param))!=0)
        {
            info_timeout_turn=atoi(info);
            set_timeout_turn(info_timeout_turn);
        }
        if((info=get_cmd_param("time_left", param))!=0)
        {
            info_time_left=atoi(info);
            set_timeout_left(info_time_left);
        }
