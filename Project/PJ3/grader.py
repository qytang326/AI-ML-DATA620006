...
grader.addBasicPart('writeupValid',
                    lambda: grader.requireIsValidPdf('blackjack.pdf'), 0)
...
class AddNoiseMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        originalSuccAndProbReward = self.originalMDP.succAndProbReward(
            state, action)
...
grader.addHiddenPart(
    ...
    3,
    description=
    "Hidden test for CounterexampleMDP. Ensure that V[startState] is greater after noise is added."
)
...
def test3a():
    ...
    tests = [([((1, None, (1, 2)), 0.5, 0), ((5, None, (2, 1)), 0.5,
                                             0)], mdp1, startState,
              'Take'), ([((0, 0, (2, 2)), 0.5, -1), ((0, 1, (2, 2)), 0.5, -1)],
                        mdp1, startState, 'Peek'), ([((0, None, None), 1, 0)],
                                                    mdp1, startState, 'Quit'),
             ([((7, None, (0, 1)), 0.5, 0), ((11, None, None), 0.5, 0)], mdp1,
              preBustState, 'Take'), ([], mdp1, postBustState, 'Take'),
             ([], mdp1, postBustState,
              'Peek'), ([], mdp1, postBustState,
                        'Quit'), ([((12, None, None), 1, 12)], mdp2,
                                  preEmptyState, 'Take')]
    for gold, mdp, state, action in tests:
        if not grader.requireIsEqual(gold, mdp.succAndProbReward(
                state, action)):
...
grader.addBasicPart(
    ...
    5,
    description=
    "Basic test for succAndProbReward() that covers several edge cases.")
...
grader.addHiddenPart(
    ...
    5,
    description=
    "Hidden test for ValueIteration. Run ValueIteration on BlackjackMDP, then test if V[startState] is correct."
)
...
def test3b():
    ...
    f = len([a for a in list(vi.pi.values())
             if a == 'Peek']) / float(len(list(vi.pi.values())))
    grader.requireIsGreaterThan(.1, f)
    for k, v in list(vi.pi.items()):
        print((k, v))
...
grader.addBasicPart(
    ...
    4,
    description=
    "Test for peekingMDP().  Ensure that in at least 10% of states, the optimal policy is to peek."
)
...
grader.addHiddenPart(
    ...
    maxSeconds=3,
    description=
    "Hidden test for incorporateFeedback(). Run QLearningAlgorithm on smallMDP, then ensure that getQ returns reasonable value."
)
...
def test4c():
    ...
    mdp.computeStates()
    rl = submission.QLearningAlgorithm(mdp.actions, mdp.discount(),
                                       submission.blackjackFeatureExtractor, 0)
    # We call this here so that the stepSize will be 1
    rl.numIters = 1
...
grader.addBasicPart(
    ...
    maxSeconds=10,
    description=
    "Basic test for blackjackFeatureExtractor.  Runs QLearningAlgorithm using blackjackFeatureExtractor, then checks to see that Q-values are correct."
)
