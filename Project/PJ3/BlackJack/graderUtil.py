...
class Grader:
    ...
    def grade(self):
        ...
        for part in parts:
            ...
            print(
                '----- END PART %s [took %s (max %s seconds), %s/%s points]' %
                (part.name, endTime - startTime, part.maxSeconds,
                 displayPoints, part.maxPoints))

        totalPoints = sum(part.points for part in parts)
        maxGradedPoints = sum(
            part.maxPoints for part in parts if self.useSolution or part.basic)
        ...
        print('========== END GRADING [%d/%d points graded]' %
              (totalPoints, maxGradedPoints))
        ...
        if len(self.manualParts) > 0:
            if not os.path.exists(self.getOutputPath('manual')):
                ...
                pass
        print("Total max points: %d" %
              (maxTotalPoints + sum(part.maxPoints
                                    for part in self.manualParts)))
