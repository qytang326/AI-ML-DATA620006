...
class CounterexampleMDP(util.MDP):
    ...
    def succAndProbReward(self, state, action):
        # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
        return [(state + 1, 0.9, 100), (state - 1, 0.1, -20)] if state == 0 else []
        # END_YOUR_CODE
    ...
    def succAndProbReward(self, state, action):
        ...
        if nextCardIndexIfPeeked != None and action == 'Take':
          ...
          if numCardsInDeck == 1 or totalCardValueInHand > self.threshold:
            ...
            reward = 0
          return [((totalCardValueInHand, None, nextdeckCardCounts), 1, reward)]
...
def MDP_simulate(mdp=smallMDP, file_name='small_result.txt', FeatureExtractor=identityFeatureExtractor):
  f = open(file_name, 'w')
  mdp.computeStates()
