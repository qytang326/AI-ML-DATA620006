...
def depthFirstSearch(problem):
    ...
    while not problem.isGoalState(
            state):  # while we do not find the goal point
        successors = problem.getSuccessors(state)  # get the point's successors
        for son in successors:
            if (not son[0] in visitedList) or (problem.isGoalState(
                    son[0]
            )):  #  if the successor has not been visited,push it into stack
...
def uniformCostSearch(problem):
    ...
    # push the starting point into queue
    fringe.push((problem.getStartState(), [], 0),
                0)  #  push starting point with priority num of 0
    # pop out the point
    (state, toDirection, toCost) = fringe.pop()
    # add the point to visited list
    ...
    while not problem.isGoalState(
        ...
        for son in successors:
            ...
            if not visitedExist:
                ...
                visitedList.append(
                    (son[0],
                     toCost + son[2]))  #  add this point to visited list
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    fringe.push(
        ...
        0 + heuristic(problem.getStartState(),
                      problem))  #  push starting point with priority num of 0
    # pop out the point
    (state, toDirection, toCost) = fringe.pop()
    # add the point to visited list
    visitedList.append((state,
                        toCost + heuristic(problem.getStartState(), problem)))
    ...
    while not problem.isGoalState(
        ...
        for son in successors:
            ...
            if not visitedExist:
                #  push the point with priority num of its total cost
                fringe.push((son[0], toDirection + [son[1]], toCost + son[2]),
                            toCost + son[2] + heuristic(son[0], problem))
                visitedList.append(
                    (son[0],
                     toCost + son[2]))  #  add this point to visited list
