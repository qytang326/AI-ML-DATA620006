...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self,
        ...
        if warn and (gameState.getNumFood() is not 1
...
class CornersProblem(search.SearchProblem):
        ...
        for action in [
            ...
            if not isWall:
                ...
                if cornerIndex < 4:  # is in one corner
                    cornerVisited_new[
                        cornerIndex] = True  # update the corner visited list
                state_new = ((x_new, y_new), cornerVisited_new)
                cost = self.costFn(x_new, y_new)
                successors.append((state_new, action,
                                   cost))  # add this successor
    ...
    def getCostOfActions(self, actions):
        ...
        cost = 0
        for action in actions:
            dx, dy = Actions.directionToVector(
                action)  # direction of the action
...
def cornersHeuristic(state, problem):
    ...
    for i in range(
            4
    ):  # there remains at most 4 corners unvisited, so we need to go for four timrs
        ...
        for cornerDiatance in cornersDistance:
            if (visitedlist[closestIndex]
                ):  # in case the first corner is visited
            ...
            if (not visitedlist[cornerIndex]) and (
                    cornerDiatance < cornersDistance[closestIndex]):
                closestIndex = cornerIndex
            cornerIndex += 1
...
def foodHeuristic(state, problem):
    ...
    # of the food point. Since we have to go to the farthest food point in order to eat out all of the food, if we use the sum of all the foods'
    # distance to be the Heuristc, it will be the minimum actual cost when all the other food points is on the way to the farthest food point. For other
    ...
    TotalDistance = sum((mazeDistance(position, foodpos,
