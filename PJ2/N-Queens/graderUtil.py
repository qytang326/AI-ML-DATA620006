...
def isEqual(trueAnswer, predAnswer, tolerance=TOLERANCE):
    ...
    if isCollection(trueAnswer) and isCollection(predAnswer) and len(
            trueAnswer) == len(predAnswer):
    ...
    if type(trueAnswer).__name__ == 'ndarray':
        import numpy as np
        if isinstance(trueAnswer, np.ndarray) and isinstance(
                predAnswer, np.ndarray):
...
class TimeoutFunction:
    ...
    def __call__(self, *args):
        if os.name == 'nt':
            ...
            if timeEnd - timeStart > datetime.timedelta(
                    seconds=self.maxSeconds + 1):
                raise TimeoutFunctionException()
            return result
...
class Grader:
    ...
    def load(self, moduleName):
        try:
            ...
            self.fail("Threw exception when importing '%s': %s" % (moduleName,
                                                                   e))
    ...
    def grade(self):
        ...
        for part in parts:
            ...
            print('----- START PART %s%s: %s' % (part.name, ' (extra credit)'
                                                 if part.extraCredit else '',
                                                 part.description))
            self.currentPart = part

            startTime = datetime.datetime.now()
            try:
                TimeoutFunction(part.gradeFunc,
                                part.maxSeconds)()  # Call the part's function
            except KeyboardInterrupt:
                raise
            except TimeoutFunctionException as e:
                self.fail(
                    'Time limit (%s seconds) exceeded.' % part.maxSeconds)
            except MemoryError as e:
                gc.collect()
                self.fail('Memory limit exceeded.')
            except Exception as e:
                self.fail('Exception thrown: %s -- %s' % (str(type(e)),
                                                          str(e)))
                self.printException()
            ...
            print('----- END PART %s [took %s (max allowed %s seconds), %s]' %
                  (part.name, endTime - startTime, part.maxSeconds,
                   displayPoints))
            print()

        activeParts = [
            part for part in parts if self.useSolution or part.basic
        ]

        totalPoints = sum(
            part.points for part in activeParts if not part.extraCredit)
        extraCredit = sum(
            part.points for part in activeParts if part.extraCredit)
        maxTotalPoints = sum(
            part.maxPoints for part in activeParts if not part.extraCredit)
        maxExtraCredit = sum(
            part.maxPoints for part in activeParts if part.extraCredit)
        ...
        lateDays = None
        if os.path.exists('metadata') and os.path.exists(
                'submit.conf') and self.params.assignmentId is not None:
            try:
                import yaml
            ...
            with open('submit.conf', 'r') as submit_conf:
                ...
                info = yaml.load(submit_conf)
            dueDate = [
                assign['dueDate'] for assign in info['assignments']
                if assign['id'] == self.params.assignmentId
            ][0]
            import dateutil.parser
            dueDate = dateutil.parser.parse(dueDate)
            dueDate = dueDate.replace(
                hour=23, minute=59)  # Choose the end of the day
            if timestamp > dueDate:
                diff = timestamp - dueDate
                lateDays = int(
                    math.ceil(diff.days + diff.seconds / (3600.0 * 24.0)))
            else:
                lateDays = 0
        ...
        if len(self.manualParts) > 0:
            if not os.path.exists(
                    self.getOutputPath('manual')) or self.params.js:
                print("Writing %d manual parts to 'grader-manual.out'" % len(
                    self.manualParts))
                result = {}
                resultParts = []
                ...
                pass
        maxBasicPoints = sum(part.maxPoints for part in self.parts
                             if part.basic and not part.extraCredit)
        maxHiddenPoints = sum(part.maxPoints for part in self.parts
                              if not part.basic and not part.extraCredit)
        maxManualPoints = sum(part.maxPoints for part in self.manualParts
                              if not part.extraCredit)
    ...
    def output(self, mode, result):
        ...
        if self.params.js:
            with open(path, 'w') as out:
                print(
                    'var ' + mode + 'Result = ' + json.dumps(result) + ';',
                    file=out)
        else:
    ...
    def requireIsValidPdf(self, path):
        ...
        if os.name == 'nt':
            ...
            if 'PDF document' not in fileType:
                return self.fail("File '%s' is not a PDF file: %s" %
                                 (path, fileType))
    ...
    def requireIsNumeric(self, answer):
        if isinstance(answer, int) or isinstance(answer, float):
            ...
            return self.fail(
                "Expected either int or float, but got '%s'" % answer)

    def requireIsOneOf(self, trueAnswers, predAnswer):
        if predAnswer in trueAnswers:
            return self.assignFullCredit()
        else:
            return self.fail("Expected one of %s, but got '%s'" % (trueAnswers,
                                                                   predAnswer))
    ...
    def requireIsEqual(self, trueAnswer, predAnswer, tolerance=TOLERANCE):
        if isEqual(trueAnswer, predAnswer, tolerance):
            ...
            return self.fail("Expected '%s', but got '%s'" % (str(trueAnswer),
                                                              str(predAnswer)))
    ...
    def printException(self):
        tb = [
            item for item in traceback.extract_tb(sys.exc_info()[2])
